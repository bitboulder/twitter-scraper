...
func (s *Scraper) GetTweet(id string, xClientTransactionID string) (*Tweet, error) {
	if s.isOpenAccount {
		...
		req, err := s.newRequest("GET", "https://x.com/i/api/graphql/zAz9764BcLZOJ0JU2wrd1A/TweetResultByRestId")
		if err != nil {
			return nil, err
		}
		...
		if xClientTransactionID == "" {
			return nil, fmt.Errorf("xClientTransactionID is required for unauthenticated requests")
		}
		...
		query.Set("features", mapToJSONString(features))
		req.URL.RawQuery = query.Encode()
		req.Header.Set("x-client-transaction-id", xClientTransactionID)
